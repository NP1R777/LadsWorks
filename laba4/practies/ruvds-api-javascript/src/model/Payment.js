...
class Payment {
     ...
     * @param dt {Date} Дата и время транзакции.
     * @param direction {module:model/Payment.DirectionEnum} * `1` - поступление средств * `2` - списание
     * @param amount {Number} Сумма в валюте пополнения `currency`.
     * @param currency {module:model/Currency} 
     * @param type {module:model/Type} 
     */
    constructor(dt, direction, amount, currency, type) { 
        
        Payment.initialize(this, dt, direction, amount, currency, type);
    }
     ...
     */
    static initialize(obj, dt, direction, amount, currency, type) { 
        obj['dt'] = dt;
    ...
    static constructFromObject(data, obj) {
        if (data) {
            ...
            if (data.hasOwnProperty('pay_source')) {
                obj['pay_source'] = ApiClient.convertToType(data['pay_source'], 'String');
            }
    ...
    static validateJSON(data) {
        ...
        if (data['pay_source'] && !(typeof data['pay_source'] === 'string' || data['pay_source'] instanceof String)) {
            throw new Error("Expected the field `pay_source` to be a primitive type in the JSON string but got " + data['pay_source']);
        }
...
Payment.RequiredProperties = ["dt", "direction", "amount", "currency", "type"];

/**
 * Дата и время транзакции.
 * @member {Date} dt
 */
Payment.prototype['dt'] = undefined;

/**
 * * `1` - поступление средств * `2` - списание
 * @member {module:model/Payment.DirectionEnum} direction
 */
Payment.prototype['direction'] = undefined;

/**
 * Дополнительная информация от платежной системы.
 * @member {String} pay_source
 */
Payment.prototype['pay_source'] = undefined;

/**
 * Сумма в валюте пополнения `currency`.
