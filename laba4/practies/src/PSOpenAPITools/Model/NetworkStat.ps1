...
function Initialize-NetworkStat {
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => NetworkStat' | Write-Debug
        ...
        if ($null -eq $InboundPps) {
            throw "invalid value for 'InboundPps', 'InboundPps' cannot be null."
        }

        if ($null -eq $OutboundPps) {
            throw "invalid value for 'OutboundPps', 'OutboundPps' cannot be null."
        }

        if ($null -eq $InboundBps) {
            throw "invalid value for 'InboundBps', 'InboundBps' cannot be null."
        }

        if ($null -eq $OutboundBps) {
            throw "invalid value for 'OutboundBps', 'OutboundBps' cannot be null."
        }


        $PSO = [PSCustomObject]@{
            "add_dt" = ${AddDt}
            "inbound_pps" = ${InboundPps}
            "outbound_pps" = ${OutboundPps}
            "inbound_bps" = ${InboundBps}
            "outbound_bps" = ${OutboundBps}
        }
...
Convert from JSON to NetworkStat<PSCustomObject>
...
NetworkStat<PSCustomObject>
#>
function ConvertFrom-JsonToNetworkStat {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => NetworkStat' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in NetworkStat
        $AllProperties = ("add_dt", "inbound_pps", "outbound_pps", "inbound_bps", "outbound_bps")
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "inbound_pps"))) {
            throw "Error! JSON cannot be serialized due to the required property 'inbound_pps' missing."
        } else {
            $InboundPps = $JsonParameters.PSobject.Properties["inbound_pps"].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "outbound_pps"))) {
            throw "Error! JSON cannot be serialized due to the required property 'outbound_pps' missing."
        } else {
            $OutboundPps = $JsonParameters.PSobject.Properties["outbound_pps"].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "inbound_bps"))) {
            throw "Error! JSON cannot be serialized due to the required property 'inbound_bps' missing."
        } else {
            $InboundBps = $JsonParameters.PSobject.Properties["inbound_bps"].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "outbound_bps"))) {
            throw "Error! JSON cannot be serialized due to the required property 'outbound_bps' missing."
        } else {
            $OutboundBps = $JsonParameters.PSobject.Properties["outbound_bps"].value
        }
