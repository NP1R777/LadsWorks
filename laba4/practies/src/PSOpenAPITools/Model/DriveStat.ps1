...
DriveStat<PSCustomObject>
#>

function Initialize-DriveStat {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => DriveStat' | Write-Debug
        ...
        if ($null -eq $ReadIops) {
            throw "invalid value for 'ReadIops', 'ReadIops' cannot be null."
        }

        if ($null -eq $WriteIops) {
            throw "invalid value for 'WriteIops', 'WriteIops' cannot be null."
        }
...
Convert from JSON to DriveStat<PSCustomObject>
...
DriveStat<PSCustomObject>
#>
function ConvertFrom-JsonToDriveStat {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => DriveStat' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in DriveStat
        $AllProperties = ("add_dt", "read_iops", "write_iops")
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "read_iops"))) {
            throw "Error! JSON cannot be serialized due to the required property 'read_iops' missing."
        } else {
            $ReadIops = $JsonParameters.PSobject.Properties["read_iops"].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "write_iops"))) {
            throw "Error! JSON cannot be serialized due to the required property 'write_iops' missing."
        } else {
            $WriteIops = $JsonParameters.PSobject.Properties["write_iops"].value
        }
