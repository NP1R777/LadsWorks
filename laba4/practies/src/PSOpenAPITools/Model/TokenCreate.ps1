...
TokenCreate<PSCustomObject>
#>

function Initialize-TokenCreate {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => TokenCreate' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        if ($null -eq $TokenName) {
            throw "invalid value for 'TokenName', 'TokenName' cannot be null."
        }

        if ($null -eq $TokenRole) {
            throw "invalid value for 'TokenRole', 'TokenRole' cannot be null."
        }
...
Convert from JSON to TokenCreate<PSCustomObject>
...
TokenCreate<PSCustomObject>
#>
function ConvertFrom-JsonToTokenCreate {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => TokenCreate' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in TokenCreate
        $AllProperties = ("token_name", "token_role", "token_expiry")
        ...
        If ([string]::IsNullOrEmpty($Json) -or $Json -eq "{}") { # empty json
            throw "Error! Empty JSON cannot be serialized due to the required property 'token_name' missing."
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "token_name"))) {
            throw "Error! JSON cannot be serialized due to the required property 'token_name' missing."
        } else {
            $TokenName = $JsonParameters.PSobject.Properties["token_name"].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "token_role"))) {
            throw "Error! JSON cannot be serialized due to the required property 'token_role' missing."
        } else {
            $TokenRole = $JsonParameters.PSobject.Properties["token_role"].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "token_expiry"))) { #optional property not found
            $TokenExpiry = $null
        } else {
            $TokenExpiry = $JsonParameters.PSobject.Properties["token_expiry"].value
        }
