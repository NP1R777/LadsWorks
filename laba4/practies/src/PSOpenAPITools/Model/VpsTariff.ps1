...
VpsTariff<PSCustomObject>
#>

function Initialize-VpsTariff {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => VpsTariff' | Write-Debug
        ...
        if ($null -eq $Vram) {
            throw "invalid value for 'Vram', 'Vram' cannot be null."
        }
...
Convert from JSON to VpsTariff<PSCustomObject>
...
VpsTariff<PSCustomObject>
#>
function ConvertFrom-JsonToVpsTariff {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => VpsTariff' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in VpsTariff
        $AllProperties = ("id", "name", "cpu", "ram", "vram", "ip", "is_active")
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "vram"))) {
            throw "Error! JSON cannot be serialized due to the required property 'vram' missing."
        } else {
