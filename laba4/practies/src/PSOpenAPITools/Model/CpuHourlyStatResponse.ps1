...
CpuHourlyStatResponse<PSCustomObject>
#>

function Initialize-CpuHourlyStatResponse {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => CpuHourlyStatResponse' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        if ($null -eq $CpuStat) {
            throw "invalid value for 'CpuStat', 'CpuStat' cannot be null."
        }
...
Convert from JSON to CpuHourlyStatResponse<PSCustomObject>
...
CpuHourlyStatResponse<PSCustomObject>
#>
function ConvertFrom-JsonToCpuHourlyStatResponse {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => CpuHourlyStatResponse' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in CpuHourlyStatResponse
        $AllProperties = ("cpu_stat")
        ...
        If ([string]::IsNullOrEmpty($Json) -or $Json -eq "{}") { # empty json
            throw "Error! Empty JSON cannot be serialized due to the required property 'cpu_stat' missing."
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "cpu_stat"))) {
            throw "Error! JSON cannot be serialized due to the required property 'cpu_stat' missing."
        } else {
            $CpuStat = $JsonParameters.PSobject.Properties["cpu_stat"].value
        }
