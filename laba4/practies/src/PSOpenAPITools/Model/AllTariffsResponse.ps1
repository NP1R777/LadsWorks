...
No description available.
.PARAMETER AdditionalService
No description available.
.OUTPUTS

AllTariffsResponse<PSCustomObject>
#>

function Initialize-AllTariffsResponse {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => AllTariffsResponse' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        if ($null -eq $Vps) {
            throw "invalid value for 'Vps', 'Vps' cannot be null."
        }
        ...
        if ($null -eq $AdditionalDrive) {
            throw "invalid value for 'AdditionalDrive', 'AdditionalDrive' cannot be null."
        }

        if ($null -eq $AdditionalService) {
            throw "invalid value for 'AdditionalService', 'AdditionalService' cannot be null."
        }
...
Convert from JSON to AllTariffsResponse<PSCustomObject>
...
AllTariffsResponse<PSCustomObject>
#>
function ConvertFrom-JsonToAllTariffsResponse {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => AllTariffsResponse' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in AllTariffsResponse
        $AllProperties = ("vps", "drive", "additional_drive", "additional_service")
        ...
        If ([string]::IsNullOrEmpty($Json) -or $Json -eq "{}") { # empty json
            throw "Error! Empty JSON cannot be serialized due to the required property 'vps' missing."
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "vps"))) {
            throw "Error! JSON cannot be serialized due to the required property 'vps' missing."
        } else {
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "additional_drive"))) {
            throw "Error! JSON cannot be serialized due to the required property 'additional_drive' missing."
        } else {
            $AdditionalDrive = $JsonParameters.PSobject.Properties["additional_drive"].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "additional_service"))) {
            throw "Error! JSON cannot be serialized due to the required property 'additional_service' missing."
        } else {
            $AdditionalService = $JsonParameters.PSobject.Properties["additional_service"].value
        }
