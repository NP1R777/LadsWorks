...
ServerNetworksResponse<PSCustomObject>
#>

function Initialize-ServerNetworksResponse {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => ServerNetworksResponse' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        if ($null -eq $V4) {
            throw "invalid value for 'V4', 'V4' cannot be null."
        }
...
Convert from JSON to ServerNetworksResponse<PSCustomObject>
...
ServerNetworksResponse<PSCustomObject>
#>
function ConvertFrom-JsonToServerNetworksResponse {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => ServerNetworksResponse' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in ServerNetworksResponse
        $AllProperties = ("v4")
        ...
        If ([string]::IsNullOrEmpty($Json) -or $Json -eq "{}") { # empty json
            throw "Error! Empty JSON cannot be serialized due to the required property 'v4' missing."
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "v4"))) {
            throw "Error! JSON cannot be serialized due to the required property 'v4' missing."
        } else {
            $V4 = $JsonParameters.PSobject.Properties["v4"].value
        }
