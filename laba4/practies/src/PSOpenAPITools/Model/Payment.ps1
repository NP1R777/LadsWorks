...
function Initialize-Payment {
    ...
    Param (
        ...
        [ValidateSet("default", "bonus", "partner")]
        [PSCustomObject]
        ${Type}
    )

    Process {
        'Creating PSCustomObject: PSOpenAPITools => Payment' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        if ($null -eq $Dt) {
            throw "invalid value for 'Dt', 'Dt' cannot be null."
        }
...
Convert from JSON to Payment<PSCustomObject>
...
Payment<PSCustomObject>
#>
function ConvertFrom-JsonToPayment {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => Payment' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in Payment
        $AllProperties = ("dt", "direction", "pay_source", "amount", "currency", "type")
        ...
        If ([string]::IsNullOrEmpty($Json) -or $Json -eq "{}") { # empty json
            throw "Error! Empty JSON cannot be serialized due to the required property 'dt' missing."
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "dt"))) {
            throw "Error! JSON cannot be serialized due to the required property 'dt' missing."
        } else {
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "pay_source"))) { #optional property not found
            $PaySource = $null
        } else {
            $PaySource = $JsonParameters.PSobject.Properties["pay_source"].value
        }
