...
DriveHourlyStatResponse<PSCustomObject>
#>

function Initialize-DriveHourlyStatResponse {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => DriveHourlyStatResponse' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        if ($null -eq $DriveStat) {
            throw "invalid value for 'DriveStat', 'DriveStat' cannot be null."
        }


        $PSO = [PSCustomObject]@{
            "drive_stat" = ${DriveStat}
        }
...
Convert from JSON to DriveHourlyStatResponse<PSCustomObject>
...
DriveHourlyStatResponse<PSCustomObject>
#>
function ConvertFrom-JsonToDriveHourlyStatResponse {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => DriveHourlyStatResponse' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in DriveHourlyStatResponse
        $AllProperties = ("drive_stat")
        ...
        If ([string]::IsNullOrEmpty($Json) -or $Json -eq "{}") { # empty json
            throw "Error! Empty JSON cannot be serialized due to the required property 'drive_stat' missing."
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "drive_stat"))) {
            throw "Error! JSON cannot be serialized due to the required property 'drive_stat' missing."
        } else {
            $DriveStat = $JsonParameters.PSobject.Properties["drive_stat"].value
        }
