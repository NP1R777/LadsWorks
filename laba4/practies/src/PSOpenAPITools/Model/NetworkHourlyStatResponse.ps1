...
.PARAMETER NetworkStat
No description available.
.OUTPUTS

NetworkHourlyStatResponse<PSCustomObject>
#>

function Initialize-NetworkHourlyStatResponse {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => NetworkHourlyStatResponse' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        if ($null -eq $NetworkStat) {
            throw "invalid value for 'NetworkStat', 'NetworkStat' cannot be null."
        }
...
Convert from JSON to NetworkHourlyStatResponse<PSCustomObject>
...
NetworkHourlyStatResponse<PSCustomObject>
#>
function ConvertFrom-JsonToNetworkHourlyStatResponse {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => NetworkHourlyStatResponse' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in NetworkHourlyStatResponse
        $AllProperties = ("network_stat")
        ...
        If ([string]::IsNullOrEmpty($Json) -or $Json -eq "{}") { # empty json
            throw "Error! Empty JSON cannot be serialized due to the required property 'network_stat' missing."
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "network_stat"))) {
            throw "Error! JSON cannot be serialized due to the required property 'network_stat' missing."
        } else {
            $NetworkStat = $JsonParameters.PSobject.Properties["network_stat"].value
        }
